...
> 2026-09-15 命名已定：底部入口「個人空間」，內部分為「我的行程／收藏景點／收藏行程／收藏資料夾」。自訂且已儲存的行程放「我的行程」，不混入「收藏行程」。本次僅新增待接入區塊與調整名稱，既有收藏操作保留。

本文件與 [saved-itinerary-handoff.md](saved-itinerary-handoff.md) 交給同一位隊友：本份處理收藏資料夾雲端 CRUD；另一份處理「個人空間 → 我的行程」清單與展示。兩者資料接口獨立，名稱與入口已確認。

更新日期：2026-09-18。對象：同時負責個人頁收藏資料夾與我的行程的開發者。

## 接手順序與責任邊界

1. 取得包含本文件與 v2 行程接口的交付分支版本；不要覆寫 profile_page.dart 已有的「個人空間」命名與「我的行程」區塊。
2. 先確認下述 ID 產生方式，再將記憶體資料夾改為雲端讀取、新增、改名、刪除與成員維護。
3. 補齊個人頁載入、空資料、失敗、未登入與帳號切換狀態；取消收藏時的成員清理行為須先協調。
4. 接續另一份文件實作「我的行程」列表與詳情，不混用收藏行程或 trips/trip_places。

我們已提供安排行程端的收藏讀取接口，隊友不必重做選擇器。若接口本身有錯或缺少約定資料，由我們修正；個人頁寫入與顯示由隊友負責。SQL 是輔助查詢／建置腳本，兩份 Markdown 才是操作與分工說明。

## 目標與狀態

使用者登入後，在個人頁建立資料夾、加入收藏景點，資料持久化到 Supabase；重開「安排行程 → 新增景點 → 我的收藏」後，可選該資料夾並看到對應景點。

依使用者提供的欄位、約束、RLS 與 policies 結果，讀取契約已確認相容，但尚未用真實帳號端到端驗證。不要將本機測試或資料庫設定確認當作雲端流程驗收；交付版本請以實際 Git 提交及推送結果為準。
